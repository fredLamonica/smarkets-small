...
import { CatalogoItem, CentroCusto, CondicaoPagamento, Endereco, GrupoCompradores, ItemSolicitacaoCompra, Iva, Moeda, OrganizacaoCompra, Pedido, PedidoItem, PessoaJuridica, SituacaoPedido, SituacaoPedidoItem, TipoEndereco, TipoFrete, TipoPedido } from '@shared/models';
import { AutenticacaoService, CentroCustoService, CondicaoPagamentoService, EnderecoService, GrupoCompradoresService, IvaService, OrganizacaoCompraService, PessoaJuridicaService, TipoPedidoService, TranslationLibraryService } from '@shared/providers';
...
@Component({
  selector: 'app-confirmar-vinculo-pedido-item',
  templateUrl: './confirmar-vinculo-pedido-item.component.html',
  styleUrls: ['./confirmar-vinculo-pedido-item.component.scss'],
})
export class ConfirmarVinculoPedidoItemComponent implements OnInit {
  ...
  confirmar() {
    if (this.isPedidoValido()) {
      ...
      this.activeModal.close(this.pedido.itens[0]);
    }
  }
  ...
  private instanciarPedido() {
    ...
    const pedidoItem = new PedidoItem(
      ...
      this.item.contratoCatalogoItem.valor,
      this.item.contratoCatalogoItem.valor * this.itemSolicitacaoCompra.quantidade,
      ...
      null,
      null,
      usuario.permissaoAtual.pessoaJuridica.idPessoaJuridica,
    );
        ...
        if (camposObrigatorios.length) {
          ...
          if (camposObrigatorios.length > 1) {
            mensagemErro = `<p>O pedido não pode ser concluído pois os campos <span class="font-weight-bold">${camposObrigatorios.join(
              ', ',
            )}</span> são obrigatorios no cadastro do produto. Favor solicitar a revisão do cadastro do produto <span class="font-weight-bold">${
