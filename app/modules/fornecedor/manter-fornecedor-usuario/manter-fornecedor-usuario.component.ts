...
import { PerfilUsuario, Permissao, PessoaFisica, SituacaoUsuario, Usuario } from '@shared/models';
...
@Component({
  ...
  selector: 'app-manter-fornecedor-usuario',
  templateUrl: './manter-fornecedor-usuario.component.html',
  styleUrls: ['./manter-fornecedor-usuario.component.scss'],
})
export class ManterFornecedorUsuarioComponent implements OnInit {
  ...
  private idTenantFornecedor: number;
  ...
  salvar() {
    ...
    if (this.form.valid) {
      ...
      if (this.idUsuario) { this.alterar(usuario); } else { this.inserir(usuario); }
    } else {
  ...
  private usuarioExistente(usuario: Usuario) {
    ...
    modalRef.result.then((result) => {
      if (result) {
        ...
        form.idPais = 30;
        const usuarioComPermissao = this.criarUsuario(form);
        this.inserir(usuarioComPermissao);
      }
    });
  }
}
