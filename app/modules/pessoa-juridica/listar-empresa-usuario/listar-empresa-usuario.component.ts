...
import { PerfilUsuario, PessoaJuridica, SituacaoUsuario, Usuario } from '@shared/models';
...
@Component({
  ...
  selector: 'listar-empresa-usuario',
  templateUrl: './listar-empresa-usuario.component.html',
  styleUrls: ['./listar-empresa-usuario.component.scss'],
})
export class ListarEmpresaUsuarioComponent implements OnInit {
  ...
  incluirUsuario() {
    const idTenantEmpresa = this.pessoaJuridica.idTenant;

    const modalRef = this.modalService.open(MaterEmpresaUsuarioComponent, {
      centered: true,
      size: 'lg',
      backdrop: 'static',
      keyboard: false,
    ...
    modalRef.componentInstance.pessoaJuridica = this.pessoaJuridica;
    modalRef.componentInstance.idTenantEmpresa = idTenantEmpresa;
  ...
  permitirEdicaoUsuario(): boolean {
    ...
    if (
      permissaoAtual.idTenant == 1 ||
      permissaoAtual.pessoaJuridica.idPessoaJuridicaMatriz == null ||
      permissaoAtual.idTenant == permissaoAtual.pessoaJuridica.idPessoaJuridicaMatriz
    ) {
      return true;
    }
  ...
  private removerVinculo(usuario: Usuario) {
    ...
    const permissao = usuario.permissoes.filter(
      permissao => permissao.pessoaJuridica.idPessoaJuridica == this.pessoaJuridica.idPessoaJuridica,
    );
