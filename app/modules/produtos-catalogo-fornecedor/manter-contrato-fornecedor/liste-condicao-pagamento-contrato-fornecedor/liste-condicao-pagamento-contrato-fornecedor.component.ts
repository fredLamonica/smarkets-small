...
import { CondicaoPagamentoService, TranslationLibraryService } from '@shared/providers';
...
@Component({
  selector: 'smk-liste-condicao-pagamento-contrato-fornecedor',
  templateUrl: './liste-condicao-pagamento-contrato-fornecedor.component.html',
  styleUrls: ['./liste-condicao-pagamento-contrato-fornecedor.component.scss'],
})
export class ListeCondicaoPagamentoContratoFornecedorComponent implements OnInit {

  @BlockUI() blockUI: NgBlockUI;
  @Input('id-contrato') idContrato: number;
  @Input('id-tenant') idTenant: number;
  ...
  obterCondicoesPagamento() {
    this.condicaoPagamentoService.listarPorTenant(this.idTenant).subscribe((response) => {
  ...
  incluir() {
    ...
    contratoCatalogoCondicaoPagamento.situacao = Situacao['Aguardando Inclusão'];
  ...
  private excluir() {
    ...
    const idsCondicaoPagamento = this.selecionadas.map((x) => x.idContratoCatalogoCondicaoPagamento);

    this.contratoCatalogoService
      .soliciteExclusaoCondicaoPagamento(this.idContrato, idsCondicaoPagamento)
      .subscribe(
        (resultado) => {
          if (resultado) {
            this.obterContratoCatalogoCondicoesPagamento();
