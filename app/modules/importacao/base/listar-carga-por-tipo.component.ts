...
export abstract class ListarCargaPorTipoComponent extends Unsubscriber implements OnInit {

  @BlockUI() blockUI: NgBlockUI;

  importations = new Array<Importacao>();
  selectedImportations = new Array<Importacao>();
  itemOrdenar: string = 'idImportacao';
      ...
      .subscribe(
        (response) => {
          if (response) {
            ...
            this.importations = new Array<Importacao>();
            this.totalPaginas = 1;
          }
          this.blockUI.stop();
        },
  ...
  agruparSelecionados(selecionados: Array<Importacao>) {
  ...
  baixarArquivo(event) {
    ...
    if (event.indexColunaExtra === 0) {
      ...
      this.arquivoService.downloadFile(
        this.importations[event.indexItem].idArquivo,
        this.importations[event.indexItem].arquivoNome).pipe(
          takeUntil(this.unsubscribe), finalize(() => this.blockUI.stop())).subscribe();
    } else {
      if (
        this.importations[event.indexItem].situacaoImportacao == SituacaoImportacao.ProcessadoErro || this.importations[event.indexItem].qtdErro > 0
      ) {
        this.blockUI.start(this.translationLibrary.translations.LOADING);

        this.importacaoService.downloadFileErro(
          this.importations[event.indexItem].idImportacao,
          'Arquivo_de_Erros_da_Importacao_' + this.importations[event.indexItem].idImportacao + '.txt').pipe(
            takeUntil(this.unsubscribe), finalize(() => this.blockUI.stop())).subscribe();
      } else {
  ...
  private configurarTabela() {
    this.settings = new CustomTableSettings(
      [
        new CustomTableColumn('Código', 'idImportacao', CustomTableColumnType.text, null, null, null),
