...
@Component({
  ...
  selector: 'app-listar-carga-clientes',
  templateUrl: './listar-carga-clientes.component.html',
  styleUrls: ['./listar-carga-clientes.component.scss'],
})
export class ListarCargaClientesComponent extends ListarCargaComponent implements OnInit {

  protected importType = ImportType.Clientes;
    ...
    this.arquivoService.inserir(arquivo[0]).subscribe(
      ...
      (response) => {
        if (response) {
          ...
          this.importacaoService.customerAddressImports(response).subscribe(
            () => {
