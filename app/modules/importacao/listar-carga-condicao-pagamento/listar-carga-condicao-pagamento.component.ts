...
@Component({
  ...
  selector: 'app-listar-carga-condicao-pagamento',
  templateUrl: './listar-carga-condicao-pagamento.component.html',
  styleUrls: ['./listar-carga-condicao-pagamento.component.scss'],
})
export class ListarCargaCondicaoPagamentoComponent extends ListarCargaComponent implements OnInit {

  protected importType = ImportType['Condições de Pagamentos'];
      ...
      this.arquivoService.inserir(arquivo[0]).subscribe(
        (response) => {
          if (response) {
            ...
            this.importacaoService.paymentTermsImport(response, this.clientSelectedIdTenant).subscribe(
