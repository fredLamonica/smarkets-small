...
@Component({
  ...
  selector: 'app-listar-carga-usuario',
  templateUrl: './listar-carga-usuario.component.html',
  styleUrls: ['./listar-carga-usuario.component.scss'],
})
export class ListarCargaUsuarioComponent extends ListarCargaComponent implements OnInit {

  protected importType = ImportType['Usuários'];
      ...
      this.arquivoService.inserir(arquivo[0]).subscribe(
        (response) => {
          if (response) {
            ...
            this.importacaoService.importUsers(response, this.clientSelectedIdTenant).subscribe(
