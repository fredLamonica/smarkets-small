...
export class PedidoEntregasProgramadasService implements EntregasProgramadasService {

  private API_URL: string = environment.apiUrl;

  constructor(private httpClient: HttpClient, private utilitiesService: UtilitiesService) { }

  get(idPedidoItem: number, itensPorPagina: number, pagina: number): Observable<Paginacao<EntregaProgramada>> {
    const params = this.utilitiesService.getHttpParamsFromObject({ itensPorPagina, pagina });

    return this.httpClient.get<Paginacao<EntregaProgramada>>(`${this.API_URL}pedidoentregasprogramadas/${idPedidoItem}`, { params }).pipe(
      map((paginacao) => {
  ...
  post(entregaProgramada: EntregaProgramada): Observable<Array<EntregaProgramada>> {
    ...
    return this.httpClient.post<Array<EntregaProgramada>>(`${this.API_URL}pedidoentregasprogramadas`, [entregaProgramadaObj.flatOut()]);
  }

  put(entregaProgramada: EntregaProgramada): Observable<Array<EntregaProgramada>> {
    const entregaProgramadaObj = new EntregaProgramada({ ...entregaProgramada });
    return this.httpClient.put<Array<EntregaProgramada>>(`${this.API_URL}pedidoentregasprogramadas`, [entregaProgramadaObj.flatOut()]);
  }
  ...
  delete(idsEntregasProgramadas: Array<number>): Observable<number> {
    ...
    return this.httpClient.delete<number>(`${this.API_URL}pedidoentregasprogramadas`, httpOptions);
  }

  getUltimaDataProgramada(idPedidoItem: number): Observable<EntregaProgramadaUltimaData> {
    return this.httpClient.get<EntregaProgramadaUltimaData>(`${this.API_URL}pedidoentregasprogramadas/${idPedidoItem}/ultimadataprogramada`);
  }
